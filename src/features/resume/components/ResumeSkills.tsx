    ...
    <section className="border-b border-neutral-200 py-8 dark:border-neutral-800 print:break-inside-avoid print:py-6">
      ...
      <div className="mt-4 grid gap-x-8 gap-y-3 sm:grid-cols-2">
        {resumeSkillGroups.map((group) => (
          <p key={group.label} className="text-sm leading-relaxed text-neutral-700 dark:text-neutral-300">
            <span className="font-semibold text-neutral-950 dark:text-neutral-50">{group.label}: </span>
            {group.items.join(", ")}
          </p>
        ))}
      </div>
